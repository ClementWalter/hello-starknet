...
func default_dict_copy_with_hint{range_check_ptr}(start: DictAccess*, end: DictAccess*) -> (DictAccess*, DictAccess*) {
    alloc_locals;
    let (new_start) = default_dict_new(0);
    let len = end - start;
    memcpy(dst=new_start, src=cast(start, felt*), len=len);
    %{
        from collections import defaultdict
        dict_tracker = __dict_manager.get_tracker(ids.end)
        new_dict_tracker = __dict_manager.get_tracker(ids.new_start)
        new_dict_tracker.current_ptr += ids.len
        new_dict_tracker.data = defaultdict(int, dict_tracker.data)
    %}
    return (new_start, new_start + len);
}
